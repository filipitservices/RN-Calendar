...
- **Edge-to-edge is on** (`edgeToEdgeEnabled=true` in `android/gradle.properties`) and `StatusBar`'s `translucent`/`backgroundColor` props were removed. Content draws behind the system bars. Native-stack **headers** own the top inset (`statusBarStyle` follows the resolved scheme). The main nav bar owns the bottom inset on Calendar and Profile. `src/ui/components/Screen.tsx` uses `react-native-safe-area-context`'s `SafeAreaView` for leftover edges only — never a hardcoded status-bar height.
...
**State ownership** is explicit. `AuthProvider` owns the session via a reducer whose state is a discriminated union (`restoring | locked | signedOut | signedIn`). Calendar mounts only for `signedIn`. The biometric prompt runs on Sign-in while `locked` (Firebase session present, app not yet unlocked). `locked` keeps the Firebase user without exposing the calendar. `EventsProvider` owns the event list.
...
- **Cold start.** Firebase restores first. If there is no user, show email/password. If there is a user but this device has no matching Keychain item, open Calendar. If there is a user **and** matching config, show Sign-in (`locked`) with the biometric button. Cancel, lockout, missing enrollment, or an invalid item fall back to password **without** calling Firebase `signOut`.
